...
@object_condition("widget", "own")
def own(ctx) -> Q:
    """Widgets I own."""
    return Q(owner=ctx.user)


@object_condition("widget", "assigned")
def assigned(ctx) -> Q:
...
@object_condition("widget", "watched_by_my_warehouse", multi_valued=True)
...
@object_condition("widget", "in_crate", params={"names": Param(list)})
...
@object_condition("crate", "named", params={"names": Param(list)})
def crate_named(ctx, *, names) -> Q:
    """Crates with one of the chosen names."""
    return Q(name__in=names)


@object_condition("widget", "my_crates")
def my_crates(ctx) -> Q:
